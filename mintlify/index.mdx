...
Yomira is not an always-on hook inside every AI agent yet. The reliable path is to install the skill or paste the setup prompt, then explicitly ask the agent to use Yomira for a decision. The agent should ask for missing information before calling the API.
